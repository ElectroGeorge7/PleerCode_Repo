/**
  ...
  *            1- DAC channel2 with DAC_OUT2 (PF1) as output   //в STM8l152c6t6 только один канал
  *
  ...
  *            - Configure DAC_OUTx (DAC_OUT1: PF0, DAC_OUT2: PF1) in analog mode.  //analog mode вроде включается для АЦП
              ...
              DAC_OutputBuffer_TypeDef DAC_OutputBuffer)
{
  ...
  //здесь используется обращение к регистрам через  адреса и указатели
  //DAC_BASE это начальный адрес блока байтов отвечающих за ЦАП (равен 0х005380, есть в даташите), 
  //к нему прибавляется отсутуп CR1_Offset чтобы перейти к регистрам CHxCR1
  //затем tmpreg2 задаёт смещение в адресе для используемого канала

  /* Get the DAC CHxCR1 value */  //получаем значение регистра CHxCR1 и записываем в переменную tmpreg
  tmpreg2 =  (uint16_t)((uint8_t)((uint8_t)DAC_Channel << 1));    
  tmpreg = *(uint8_t*)((uint16_t)(DAC_BASE + CR1_Offset + tmpreg2));  //полученное в скобках ((uint16_t)(DAC_BASE + CR1_Offset + tmpreg2)) значение является адресом на 
                                                                      //нужный нам регистр CHxCR1
                                                                      //uint8_t* показывает что в следующих скобках указатель на байт - 8 битный регистр(тип адрес) 
                                                                      //а звездочка после равно нужна, чтобы получить значение по указанному адресу(указателю)
  ...
  /* Write to DAC CHxCR1 */
  *(uint8_t*)((uint16_t)(DAC_BASE + CR1_Offset + tmpreg2)) = (uint8_t)tmpreg; //записываем в регистр значение tmpreg
}
